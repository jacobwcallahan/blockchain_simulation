...
class Stats:
    ...
    def set_inflation(self, time_since_last_print):
        ...
        new_coins = self.blockchain.coins - self.print_dict['last_coins']

        coins_per_second = new_coins / time_since_last_print

        if time_since_last_print == 0 or self.print_dict["coins"] == 0:
            self.print_dict["inflation"] = 0
        else:
            self.print_dict["inflation"] = ((secs_in_year * coins_per_second) / self.blockchain.coins) * 100

        self.print_dict["last_coins"] = self.blockchain.coins
